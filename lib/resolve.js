...
class ProxyResolverWindows extends ProxyResolver {
  constructor(options = {}) {
    ...
    proxies.forEach(p => {
      const match = p.match(/((?<scheme1>\w+)=)?((?<scheme2>\w+):[/][/])?((?<username>[^:@]+)(:(?<password>[^@]+))?@)?(?<host>[^: ]+)(:(?<port>\d+))?/);
      if (match) {
        const { scheme1, scheme2, host, port } = match.groups;
        const protocol = `${scheme2 || scheme1 || 'http'}:`;
...
class ProxyResolverPAC extends ProxyResolver {
  ...
  resolve(targetUrl) {
    if (!this.pac || this.isBypass(targetUrl)) {
      return Promise.resolve(null);
    }
    try {
      return pacResolver(this.pac)(targetUrl)
        .catch(() => null)
        .then(res => res ? res.split(/;/) : null);
    } catch (e) {
      debug(`proxy PAC error for ${targetUrl}: ${e.message}`);
      return Promise.resolve(null);
    }
  }
}
      ...
      new Promise((resolve, reject) => {
        http.get(u, res => {
          ...
          res.on('end', () => {
            ...
            resolve({ headers, statusCode, body: rawData });
          });
        }).on('error', e => reject(e));
      })
    );
    ...
    return loadPac(proxy.url, options)
      .then(res => {
        if (res) {
          ...
          if (statusCode < 300 && body && headers) {
            const contentType = headers['content-type'];
            if (contentType === 'application/x-ns-proxy-autoconfig') {
              return new ProxyResolverPAC({ pac: body, bypass: proxy.bypass });
            }
            debug(`'proxy PAC invalid mime-type: ${contentType}`);
          }
          debug(`'proxy PAC download error (${statusCode}): ${body}`);
        }
